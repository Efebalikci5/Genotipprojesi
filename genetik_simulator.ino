...
void loop() {
    ...
    if (digitalRead(anneButon) == LOW && millis() - lastPress[0] > 1000) {
        lastPress[0] = millis();
        // Anne genotipini rastgele değiştir
        anne.gen1 = random(2) ? 'A' : 'a';
        anne.gen2 = random(2) ? 'A' : 'a';
        gosterGenotip(0, anne);
        // Çocukları yeniden hesapla
        hesaplaVeGosterCocuklar();
        Serial.println("Anne genotipi değiştirildi");
    }
    // Baba butonu kontrolü
    if (digitalRead(babaButon) == LOW && millis() - lastPress[1] > 1000) {
        lastPress[1] = millis();
        // Baba genotipini rastgele değiştir
        baba.gen1 = random(2) ? 'A' : 'a';
        baba.gen2 = random(2) ? 'A' : 'a';
        gosterGenotip(1, baba);
        // Çocukları yeniden hesapla
        hesaplaVeGosterCocuklar();
        Serial.println("Baba genotipi değiştirildi");
    }
}
